...
<div class="container-xl">
  <div class="row g-2 align-items-center mb-3">
    ...
    <div class="col-auto ms-auto">
      <form action="{{ route('admin.dashboard') }}" method="GET" id="filterForm" class="ms-auto">
        <div class="d-flex gap-1">
          <select class="form-select" name="lokasi" onchange="document.getElementById('filterForm').submit();">
        ...
        tooltip: {
          callbacks: {
            label: function(context) {
              let total = dataMasuk.reduce((a, b) => a + b, 0);
    ...
    options: {
      plugins: {
        tooltip: {
          callbacks: {
            label: function(context) {
              let total = dataPulang.reduce((a, b) => a + b, 0);
              let value = context.raw;
              let percent = total ? ((value / total) * 100).toFixed(1) : 0;
              return `${value} (${percent}%)`;
            }
