...
<div style="border-radius: 70px; border-bottom-left-radius: 0; border-bottom-right-radius: 0;" class="bg-white p-0 px-5 py-5 vh-100">
  <div class="row g-2 align-items-center mb-3">
    ...
    <div class="col-auto ms-auto">
      <div class="btn-list">
        <a href="{{ route('user.izin.index') }}" class="btn btn-danger rounded-pill px-3">Back</a>
      </div>
    </div>
  </div>
  <div class="row">
    ...
    <form action="{{ route('user.izin.update', $izin->id) }}" method="POST" class="" enctype="multipart/form-data">
      ...
      <div class="mb-3">
        ...
        <select class="form-select" name="alasan">
          ...
          <option value="tidak-masuk" @if($izin->alasan == 'terlambat') @selected(true) @endif>Tidak Masuk</option>
          <option value="terlambat" @if($izin->alasan == 'terlambat') @selected(true) @endif>Terlambat</option>
        </select>
        @error('sampai')<div class="text-danger">{{ $message }}</div>@enderror
      </div>
      <div class="mb-3">
